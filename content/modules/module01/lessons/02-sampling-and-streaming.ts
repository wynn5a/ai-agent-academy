...
for event in stream:
    ...
    },
    {
      ...
      answer:
        "**Core:** At each step the model produces logits over the vocabulary. Temperature **divides the logits before softmax**: T < 1 sharpens the distribution (probability mass concentrates on the top tokens), T > 1 flattens it (tail tokens become viable), T → 0 approaches argmax. It doesn't change what the model *knows* — it changes how the next token is *picked* from what the model already believes. | **Senior add-ons:** T=0 still isn't perfectly deterministic (GPU non-determinism, ties, batching effects), and temperature interacts multiplicatively with top_p, which is why you tune one, not both. | **Follow-up probe:** \"so why did frontier models remove it?\" → the useful control turned out to be *how much work* the model does (thinking/effort), not the shape of the token distribution — and prompting steers style better than distribution-flattening does.",
    },
    {
      type: "exercise",
      kind: "concept",
    ...
    },
    {
      ...
      answer:
        "**Core:** Move the variety into the **prompt and the harness**. Describe the variety you want ('propose 3 directions with distinct voices, avoid the phrasing of previous outputs'), inject variation deliberately (rotate style exemplars, seeds of context, persona instructions per request), or generate N candidates and pick/judge. On models that still expose temperature, you'd raise it — but say explicitly that prompt-level steering usually beats distribution-flattening even there, because temperature adds *randomness*, not *taste*. | **Follow-up probe:** \"how do you evaluate that the outputs actually got more varied?\" → embedding-distance or n-gram overlap across samples, plus human preference evals — which is Module 7's territory, and saying so shows you know the map.",
    },
    {
      type: "keypoints",
      points: [
