...
- `lib/types.ts` — content schema, including `GATES` (checkpoint requirements) and `PASS_THRESHOLD` (0.8). Code sections support dual-provider tabs: `provider: "claude" | "openai" | "neutral"` plus `variants: [{ provider, code, explanation? }]` — when `variants` is present the UI renders Claude/OpenAI tabs synced site-wide. Callout kind `"career"` renders as a "Hiring signal" callout (content decisions align with `../ai-agent-engineer-hiring-research-2026.md`, one level above this repo).
- `lib/provider.ts` — global Claude/OpenAI code-example preference (`localStorage` key `aea-provider-v1`, `useSyncExternalStore`); toggled from the sidebar or any tabbed code block.
